...
```{r lines-plot, include=FALSE}
#because I'm reloading different versions of plyr,
#have to avoid ggplot because of its dependence on plyr. Unf.
lines_plot <- function(..., data, group, legend.loc="topleft", method=plot) {
  splits <- split(data, eval(substitute(group), data, parent.frame()))
  colors <- rainbow(length(splits), v=0.7)
  method(..., data=data, type="n")
  for (i in seq_along(splits)){
    lines(..., data = splits[[i]], type="l", new=FALSE, col=colors[[i]])
  }
  legend(legend.loc, legend=names(splits), col=colors, lty=1,
         title=deparse(substitute(group)))
}
```
...
```{r with-par, include=FALSE}
with_par <- function(list, expr) {
  old <- par(names(list))
  par(list)
  on.exit(par(old))
  expr
...
```{r plot-subassignment-performance}
...
```{r revgetter, cache.extra = mapply(digest::digest, file=paste0("~/plyr/.git/refs/heads/", c("master","fastrbind")))}
...
```{r gather-revs, results="hide", message=FALSE}
rev.data <- Map(
    rev=revs, try_rev, 
